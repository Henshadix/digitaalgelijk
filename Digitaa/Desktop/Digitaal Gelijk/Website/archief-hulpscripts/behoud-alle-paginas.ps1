Write-Host "===== Website opschonen (behoud alle pagina's) =====" -ForegroundColor Cyan
Write-Host ""
...
$overbodige_bestanden = @(
    ...
    # Instructie bestanden die niet nodig zijn in productie
    "*INSTRUCTIES*.md",
    "*INSTRUCTIES*.txt",
    "EASYPANEL-*.md",
    "HOSTING.md",
...
$overbodige_mappen = @(
    # Lege lijst - we behouden alle mappen om er zeker van te zijn dat we geen pagina's verliezen
)
...
# Toon de resultaten
Write-Host "Gevonden overbodige bestanden (die veilig verwijderd kunnen worden):" -ForegroundColor Yellow
foreach ($bestand in $overbodige_bestanden_gevonden) {
    Write-Host "  - $bestand" -ForegroundColor Gray
}

Write-Host "`nGevonden overbodige mappen (die veilig verwijderd kunnen worden):" -ForegroundColor Yellow
if ($overbodige_mappen_gevonden.Count -eq 0) {
    Write-Host "  Geen overbodige mappen gevonden. Alle mappen worden behouden om pagina's te bewaren." -ForegroundColor Gray
} else {
...
if ($antwoord -eq "j" -or $antwoord -eq "J") {
    ...
    Write-Host "`nAlle overbodige bestanden zijn verplaatst naar: $archiveDir" -ForegroundColor Green
